...
import { specializationIcons } from './constants'
import {
  Badge,
  Separator,
  Tabs,
...
export const SpecializationCard = ({ specialty }: { specialty: CatalogSpecialty }) => {
  const defaultProgram = specialty.programs[0]
  const [activeEducationLevel, setActiveEducationLevel] = useState(defaultProgram?.educationLevel)

  const activeProgram =
    specialty.programs.find((program) => program.educationLevel === activeEducationLevel) ??
    defaultProgram

  const Icon = specializationIcons[specialty.code] ?? HelpCircleIcon
    ...
    <article className="border-border bg-card flex h-full flex-col rounded-xl border p-5 md:p-6">
      <header>
        ...
        <Typography
          ...
          variant="title-lg"
          render={
            !!activeProgram && <Link href={`/specializations/${activeProgram.slug}` as Route} />
          }
          className="hover:text-accent-violet mt-5 underline underline-offset-3 transition-colors"
        >
          {specialty.title}
        </Typography>
        <Typography
