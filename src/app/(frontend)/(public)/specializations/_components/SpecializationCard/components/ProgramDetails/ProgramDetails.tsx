...
import type { CatalogProgram } from '../../../../_types'
...
export const ProgramDetails = ({ program }: { program: CatalogProgram }) => {
  if (!program.forms.length) {
    return (
      <Typography
        as="p"
        variant="body-sm"
  ...
  return (
    <div className={cn('mt-4 border-y', program.forms.length > 1 && 'grid md:grid-cols-2')}>
      {program.forms.map((form, index) => (
        <section
          key={form.form}
          aria-label={`${studyFormLabels[form.form]} форма`}
          className={cn(
            'py-5',
            index > 0 && 'border-t pt-5 md:border-t-0 md:border-l md:pt-5 md:pl-5',
            program.forms.length > 1 && !(index > 0) && 'md:pr-5'
          )}
        >
          <Typography
            as="h4"
          ...
          <dl className="mt-3">
            ...
            <StudyFact
              label="За рік"
              value={formatTuition(form.tuition)}
            />
            <StudyFact
              label="Ліцензійний обсяг"
              value={formatPlaces(form.admission.licensedCapacity)}
            />
            <StudyFact
              label="Держзамовлення"
              value={formatPlaces(form.admission.maxStateOrder)}
            />
          </dl>

          {/* <AdmissionStats form={form} /> */}
          <section
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              >
                Середній бал (вступ {form.admission.statisticsYear ?? 'попереднього року'})
              </Typography>

              {form.admission.statisticsUrl && (
                <a
                  href={form.admission.statisticsUrl}
                  target="_blank"
            ...
            <dl className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
              <div className="flex items-baseline gap-1.5">
                ...
                <dd className="font-jetbrains text-xs font-semibold">
                  {formatScore(form.admission.averageBudgetScore)}
                </dd>
              </div>
              <div className="flex items-baseline gap-1.5">
                <dt className="text-muted-foreground text-xs">Контракт</dt>
                <dd className="font-jetbrains text-xs font-semibold">
                  {formatScore(form.admission.averageContractScore)}
                </dd>
              </div>
            </dl>
          </section>
