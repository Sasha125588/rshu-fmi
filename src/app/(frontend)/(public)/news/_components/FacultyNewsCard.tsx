...
export const FacultyNewsCard = ({
  ...
  const shareCover = variant !== 'compact' && !!image
      ...
      <article>
        ...
        >
          {variant !== 'compact' &&
            (shareCover ? (
              <ViewTransition
                name={`faculty-news-cover-${item.id}`}
                share="news-cover"
                default="none"
              >
                {media}
          ...
          <span className="min-w-0">
            <NewsCardMeta>
              ...
              {item.isPinned && (
                <ViewTransition
                  name={`faculty-news-pinned-${item.id}`}
                  share="news-meta"
                  default="none"
                >
                  <Badge
              ...
              <ViewTransition
                name={`faculty-news-date-${item.id}`}
                share="news-meta"
                default="none"
              >
                <Typography
              ...
              {!!tagLabels.length && (
                <ViewTransition
                  name={`faculty-news-tags-${item.id}`}
                  share="news-meta"
                  default="none"
                >
                  <NewsCardTags tags={tagLabels} />
                </ViewTransition>
              )}
            </NewsCardMeta>

            <ViewTransition
              name={`faculty-news-title-${item.id}`}
              share="news-title"
              default="none"
            >
            ...
            <ViewTransition
              name={`faculty-news-excerpt-${item.id}`}
              share="news-excerpt"
              default="none"
            >
